...
window.onload = function(){
	let size = ["small", "middle", "large"];
	const PIZZAS = [
		{name: 'margarita', size: size, cost: [2.50, 3.00, 3.50], availableExtras: ['Mozzarella', 'tomatoes', 'basil', 'olives', 'beef', 'bacon', 'mashrooms', 'chicken', 'onion', 'oregano', 'hot red peper']},
		{name: 'peperoni', size: size, cost: [3.00, 3.50, 4.00], availableExtras: ['Mozzarella', 'tomatoes', 'basil', 'olives', 'beef', 'bacon', 'mashrooms', 'chicken', 'onion', 'oregano', 'hot red peper']},
		{name: 'napoli', size: size, cost: [3.25, 3.75, 4.25], availableExtras: ['Mozzarella', 'tomatoes', 'basil', 'olives', 'beef', 'bacon', 'mashrooms', 'chicken', 'onion', 'oregano', 'hot red peper']},
		{name: 'fourSeasons', size: size, cost: [3.50, 4.00, 4.50], availableExtras: ['Mozzarella', 'tomatoes', 'basil', 'olives', 'beef', 'bacon', 'mashrooms', 'chicken', 'onion', 'oregano', 'hot red peper']},
		{name: 'calzone', size: size, cost: [3.15, 3.65, 4.15], availableExtras: ['Mozzarella', 'tomatoes', 'basil', 'olives', 'beef', 'bacon', 'mashrooms', 'chicken', 'onion', 'oregano']}
	]
	const EXTRAS = [
...
//RENDERING:
	function renderPizza(pizzaData, pizzaNode) {
		let templ = document.getElementById("myTemplate");
		let unit = templ.content;
		let pizzas = document.getElementById("pizzas");
		
		var i = 0, max = PIZZAS.length;
		for(; i < max; i++){
			let newUnit = unit.cloneNode(true);
			pizzas.appendChild(newUnit);
		}
		let units = document.querySelectorAll('.unit');
		var i = 0; max = units.length;
		for(; i < max; i++){
			units[i].setAttribute("id", PIZZAS[i].name +"Unit");
			units[i].children[0].textContent = PIZZAS[i].name;
			units[i].children[1].setAttribute("id", PIZZAS[i].name + "Pic");
			let j = 0, maxS = size.length;
			for(; j < maxS; j++){
				var inpt = document.createElement("input");
				inpt.setAttribute("value", PIZZAS[i].size[j]);
				inpt.setAttribute("type", "button");
				inpt.setAttribute("class", "sizeButton btn btn-default");
				units[i].children[2].appendChild(inpt);
			}
			units[i].children[3].children[1].setAttribute("id", PIZZAS[i].name +"Numb");
			units[i].children[3].children[1].textContent = "0";
			units[i].children[3].children[3].children[0].setAttribute("id", PIZZAS[i].name + "Cost");
			units[i].children[3].children[3].children[0].textContent = "0";
			units[i].children[3].children[3].children[1].textContent = "$";
			units[i].children[4].setAttribute("id", PIZZAS[i].name + "Ext");
			units[i].children[4].children[0].textContent = "0.3$-0.5$ per one extra";
			units[i].children[4].children[1].setAttribute("id", PIZZAS[i].name + "E");
			units[i].children[4].children[2].setAttribute("id", PIZZAS[i].name + "Drop");
			let k = 0, maxK = EXTRAS.length;
			for(; k < maxK; k++) {
				var li = document.createElement("li");
				li.setAttribute("name", EXTRAS[k]);
				li.setAttribute("type", "button");
				li.setAttribute("class", "adds btn btn-default btn-xs");
				li.textContent = EXTRAS[k].name;
				units[i].children[4].children[2].appendChild(li);
			}
		}
	}
	renderPizza();
	var dropdown = function () {
		...
		else if (getComputedStyle(ul).visibility === "visible") {
			ul.style.visibility = "hidden";
		}
	}
	const carets = document.querySelectorAll(".caret");
	carets.forEach(caret => caret.addEventListener('click', dropdown));
}
//MANIPULATING
	// var redactButs;
